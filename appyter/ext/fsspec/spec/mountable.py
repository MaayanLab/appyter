...
class MountableAbstractFileSystem:
  ...
  def mount(self: AbstractFileSystem, path='', mount_dir: t.Optional[pathlib.Path]=None, fuse=False, **kwargs):
    ...
    '''
    if path:
      # let chrootfilesystem deal with path fixup
      from appyter.ext.fsspec.chroot import ChrootFileSystem
      return ChrootFileSystem(fs=self, fo=path).mount(mount_dir=mount_dir, fuse=fuse, **kwargs)
    else:
      logger.debug(f"{self} mount on {mount_dir} ({fuse=})")
      if fuse:
