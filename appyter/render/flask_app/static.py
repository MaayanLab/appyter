...
def get_index():
  ...
  if mimetype in {'text/html'}:
    ...
    return send_file(get_ipynb_io(), as_attachment=True, download_name=current_app.config['IPYNB'], mimetype=mimetype, conditional=True)
  else:
    abort(404)
...
def favicon():
  ...
  if static.exists('favicon.ico'):
    return send_file(static.open('favicon.ico', 'rb'), as_attachment=True, download_name='favicon.ico', conditional=True)
  abort(404)
...
def static(filename):
  ...
  if static.exists(filename):
    return send_file(static.open(filename, 'rb'), as_attachment=True, download_name=filename, conditional=True)
    #
  try:
  ...
  if path.endswith('/'):
    ...
    if mimetype == 'text/html':
      ...
      if output_fs.exists(path):
        response = send_file(output_fs.open(path, 'rb'), as_attachment=True, download_name=current_app.config['IPYNB'], conditional=True)
    ...
    output_fs = output_fs_ctx.open()
    if output_fs.exists(path):
      response = send_file(output_fs.open(path, 'rb'), as_attachment=True, download_name=os.path.basename(path), conditional=True)
      response.call_on_close(output_fs_ctx.close)
      return response
    else:
      output_fs_ctx.close()
